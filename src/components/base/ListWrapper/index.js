...
const ListWrapper = ({
  ...
  const _renderItem = ({ item, index }) => {
    return renderItem ? renderItem({ item, index }) : cloneElement(children, { item, index });
  };
  ...
  const _renderSeparator = () => {
    return horizontal ? <Block width={HSeparator} /> : <Block height={VSeparator} />;
  };

  const _renderLoadMore = () => {
    return <Block marginVertical={12}>{/* <BallIndicator size={27} color={COLORS.text} /> */}</Block>;
  };
  ...
  return (
    <FlatList
      ...
      contentContainerStyle={{
        ...
        paddingBottom: safeAreaBottom ? (typeof safeAreaBottom === 'number' ? safeAreaBottom : bottom) : 0,
        ...contentContainerStyle,
      }}
      ref={setRef}
